...
type Invite = { code: string; expiresAt: string; claimedAt: string | null };
...
export default function StaffReviews({admin=false}:{admin?:boolean}) {
  ...
  return <div className="space-y-6">
    ...
    {!admin && <section className="admin-panel p-5 space-y-4">
      ...
      <p className="text-sm">Ask the customer to open this page on their own phone and enter your code:</p>
      <div className="flex flex-wrap gap-3 items-center"><span className="text-brand break-all">{customerUrl || "/review"}</span><button className="admin-button" onClick={() => {void action.run(async()=>{await navigator.clipboard.writeText(customerUrl);setCopied(true);});}}>{copied ? "Link copied" : "Copy link"}</button></div>
      {invite && remaining > 0 ? <div className="rounded-xl bg-brand/5 p-5 text-center space-y-2">
        {invite.claimedAt ? <><p className="font-semibold">Customer is completing the review</p><p className="text-sm text-foreground/60">This code has been used. Their review can be submitted once.</p></> : <><p className="text-sm">Your single-use code</p><p className="text-5xl tracking-[0.25em] font-mono font-bold text-brand" aria-label={`Review code ${invite.code}`}>{invite.code}</p><p className="text-sm tabular-nums">Expires in {Math.floor(remaining/60)}:{String(remaining%60).padStart(2,"0")}</p></>}
      </div> : <><button className="admin-button" disabled={loading || action.busy} onClick={() => {void action.run(async()=>{const result=await api<{invite:Invite;serverTime:string}>("/api/team/reviews","POST",{});setFresh({...result,receivedAt:Date.now()});});}}>{action.busy ? "Generating…" : "Generate 4-digit code"}</button><p className="text-xs text-foreground/60">Valid for five minutes. Each code is linked only to you and accepts one customer review.</p></>}
