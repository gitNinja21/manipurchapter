      ...
      {!admin && (
        <details className="admin-panel p-5" open={["LATE_ARRIVAL", "SHIFT_CHANGE"].includes(params.get("kind") ?? "") || undefined}>
          ...
          >
            ...
            {kind === "SHIFT_CHANGE" && <p className="text-sm text-foreground/60">Normal clock-in opens 15 minutes before your start. Earlier starts and sudden shift changes can be requested immediately, including today. Only an approved request changes your shift for this date. For today’s replacement shifts, an admin can approve after the shift starts or after you clock in. Approval updates your scheduled start, not your recorded clock-in/out times. If your actual arrival was not recorded, request an attendance correction. Use a 9-hour interval including the paid break, or 7 hours for Tokili on Friday. Required finish is measured from actual clock-in.</p>}
