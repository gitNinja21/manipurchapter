...
export default function AttendanceTracking() {
  ...
  const historical=!usesMasterPolicy(range.to);
  ...
  return <section className="space-y-5">
    <div><h2 className="text-xl font-semibold">Attendance tracking</h2><p className="text-sm text-foreground/65 mt-1">Date-wise late arrivals and early departures. Times are in IST.</p></div>
    <div className="admin-panel p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <label className="text-sm">Show<select className="input mt-1" value={filter} onChange={e=>{setFilter(e.target.value);setPage(1);}}>{[["attention","Needs attention"],["all","All attendance"],["late","Late incidents (>15 min)"],["early","Early departures"],...(historical ? [["pending","Historical pending talks"]] : [])].map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
    ...
    {data && <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[["Late arrivals in range",selected.filter(r=>r.lateIncident).length],["Employees late in range",new Set(selected.filter(r=>r.lateIncident).map(r=>r.userId)).size],["Late on end date",lastDay.filter(r=>r.lateIncident).length],["Left early on end date",lastDay.filter(r=>r.earlyIncident).length]].map(([label,value])=><div key={label} className="admin-panel p-4"><p className="text-xs text-foreground/65">{label}</p><p className="text-2xl font-semibold mt-1">{value}</p></div>)}
      </div>
      ...
      <p className="text-xs text-foreground/65">From 26 September, late time excludes 15 minutes of paid grace. Monthly late counts and award eligibility are shown in Performance. Manager-talk rules apply only to historical records.</p>
      <div className="admin-panel overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm min-w-[1050px]">
        <thead><tr>{["Date / employee","Clock-in / finish","Late arrival","Early departure",historical ? "Consecutive days" : "Monthly attendance",historical ? "Manager talk" : "Details"].map(label=><th key={label} className="text-left px-4 py-3">{label}</th>)}</tr></thead>
        <tbody>{rows.slice((currentPage-1)*50,currentPage*50).map(r=><tr key={`${r.userId}:${r.date}`} className="border-t border-border align-top">
          ...
          <td className={`px-4 py-3 ${r.lateIncident ? "text-danger" : ""}`}>{r.clockInAt ? minutes(r.lateMinutes) : "—"}{r.lateExcused ? <p className="text-xs">Excused · not counted</p> : !usesMasterPolicy(r.date) && r.lateMinutes>0 && r.lateMinutes<=15 ? <p className="text-xs">Within 15-minute grace</p> : null}</td>
          ...
          <td className="px-4 py-3">{usesMasterPolicy(r.date) ? <Link href={manage(r.userId).replace("section=clearance","section=points")} className="text-brand underline">View monthly late count & award eligibility</Link> : <><p>Late: <strong>{r.lateStreak}</strong> days</p><p>Early: <strong>{r.earlyStreak}</strong> days</p><p className="text-xs text-foreground/60">As of this date</p></>}</td>
          <td className="px-4 py-3">{!usesMasterPolicy(r.date) && <><p>Late: {talkLabel[r.lateTalk]}</p><p>Early: {talkLabel[r.earlyTalk]}</p>{[r.lateTalk,r.earlyTalk].some(s=>s!=="NONE") && <Link href={manage(r.userId)} className="text-brand underline text-xs">View talks & points</Link>}</>}{r.recordId && <Link href={`/admin/attendance?from=${r.date}&to=${r.date}&userId=${r.userId}`} className="text-brand underline text-xs block mt-1">View attendance</Link>}</td>
        </tr>)}</tbody>
