...
test("clock-out reminders start at 22:30 even for a later scheduled finish",()=>{
  ...
  assert.equal(attendanceReminder({...base,open:{...open,scheduledEndAt:at("23:00")},now:at("22:30")})?.kind,"ATTENDANCE_OUT");
...
test("overnight shifts retain the prior day's deadline after midnight",()=>{
  ...
  assert.equal(attendanceReminder({...base,open,now:new Date("2026-09-20T00:29:00+05:30")})?.kind,"ATTENDANCE_OUT");
