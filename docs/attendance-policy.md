...
After a failed location lookup, employees can choose “Can’t get location? Ask manager to verify” and take a face-matched selfie for clock-in or clock-out. The server timestamps the request; no attendance is completed until approval. Requests and protected selfie links appear under Team → Requests. Admins can review later from anywhere; being on site at submission is not required. Approval requires a review confirmation and note, uses the saved submission time, and records an attendance audit. Duplicate pending requests, changed attendance, leave, unopened/unassigned shifts and shifts over 24 hours are blocked. Pending manager meetings must be cleared first; late clock-out hours still require their separate review. Rejecting or cancelling a request does not change attendance. Deploy migration `20260924090000_location_fallback` before running the new app.

Meeting clearance and arrival approval are separate admin actions in Performance → Manager clearance. Confirming a completed meeting requires an outcome note but no arrival attempt, and never approves an arrival. A separate arrival form approves today’s recorded or verified claimed time. When a held arrival exists, clock-in waits for its approval so the waiting time is preserved; without a held arrival, the employee can clock in normally after all meetings are cleared.
